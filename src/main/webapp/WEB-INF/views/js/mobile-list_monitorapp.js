...
$(document).on('click', '.product-row-add-btn', function() {
	...
	var part_cnt	= 0;
	$('input[name^=p_pr_code]').each(function(index) {
	    part_code += '<li>' + 
		'<input type="hidden" name="part_pr_code[]" value="' + $('input[name^=p_pr_code]').eq(index).val() + '" />' + 
		'<input type="hidden" name="part_price[]" value="' + $('input[name^=p_price]').eq(index).val() + '" />' + 
		'<input type="hidden" name="part_quantity[]" value="' + $('input[name^=p_quantity]').eq(index).val() + '" />' + 
		'<h3>' + $('input[name^=p_pr_name]').eq(index).val() + '</h3>' +
		JS_TEXT['price'] + ":" + $('input[name^=p_price]').eq(index).val() + '<br>' +
		JS_TEXT['quantity'] + ":" + $('input[name^=p_quantity]').eq(index).val() +
		'</li>';
	    part_cnt++;
	});
	code = '<div class="list-content-wrapper">' +
		'<div class="list-content">' + 
		    '<input type="hidden" name="part_cnt[]" value="' + part_cnt + '" />' + 
		    '<input type="hidden" name="sales_type[]" value="' + $('select[name=product_sales_type]').val() + '" />' + 
...
$(document).on('change', 'select[name=product_sales_type]', function() {
    _ajax_html('/sfa/product/xhr/get_p_class.xhr.php', 'sales_type=' + $(this).val(), print_p_class_option);
});
function print_p_class_option(data) {
    var target  = $('select[name=product_class]');
    target.html(data);
}
